...
class Tree_Table(forms.TreeGridView):
    ...
    def create_constraint_edge_table(cls, sceneNode, dual=[]):
        ...
        Allowed = ["target_vector", "f", "target_force"]
        attributes = filter(lambda attr: attr in Allowed, attributes)
    ...
    def create_constraint_formvertex_table(cls, sceneNode, dual=[]):
        ...
        Allowed = ["line_constraint", "is_fixed"]
        attributes = filter(lambda attr: attr in Allowed, attributes)
    ...
    @classmethod
    def create_constraint_forcevertex_table(cls, sceneNode, dual=[]):
...
class Tree_Tab(forms.TabPage):
    ...
    @classmethod
    def from_edgeconstraints(cls, sceneNode, tab_type, dual=None):
        tab = cls()
        tab.Text = tab_type
        # create_table = getattr(Tree_Table, "create_%s_table" % tab_type)
        tab.Content = Tree_Table.create_constraint_edge_table(sceneNode, dual=dual)
        return tab

    @classmethod
    def from_vertexconstraints(cls, sceneNode, tab_type, dual=None):
        tab = cls()
        tab.Text = tab_type
        # create_table = getattr(Tree_Table, "create_%s_table" % tab_type)
        tab.Content = Tree_Table.create_constraint_formvertex_table(sceneNode, dual=dual)
        return tab

    @classmethod
    def from_dualvertexconstraints(cls, sceneNode, tab_type, dual=None):
        tab = cls()
        tab.Text = tab_type
        # create_table = getattr(Tree_Table, "create_%s_table" % tab_type)
        tab.Content = Tree_Table.create_constraint_forcevertex_table(sceneNode, dual=dual)
        return tab
